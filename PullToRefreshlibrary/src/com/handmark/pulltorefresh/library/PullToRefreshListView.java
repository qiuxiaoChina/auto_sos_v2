...
public class PullToRefreshListView extends PullToRefreshAdapterViewBase<ListView> {
    ...
    protected void onReset() {
        ...
        switch (getCurrentMode()) {
            ...
            case PULL_FROM_END:
                ...
                scrollLvToEdge = Math.abs(mRefreshableView.getLastVisiblePosition() - selection)
                        <= 1;
                break;
            case PULL_FROM_START:
            default:
                ...
                selection = 0;
                scrollLvToEdge = Math.abs(mRefreshableView.getFirstVisiblePosition() - selection)
                        <= 1;
                break;
        }
    ...
    protected void handleStyledAttributes(TypedArray a) {
        ...
        if (mListViewExtrasEnabled) {
            final FrameLayout.LayoutParams lp = new FrameLayout.LayoutParams(FrameLayout
            ...
            if (scrollView != null) {
                switch (event.getAction()) {
                    case MotionEvent.ACTION_DOWN:
                        ...
                        break;
                    default:
                        touchUp = currentY - event.getY() > 0;
                        break;
                }
                if (!touchUp && (getChildAt(0).getTop() < 0 || getFirstVisiblePosition() > 0)) {
                    disallow = true;
                } else if (scrollView.getScrollY() >= scrollHeight) {
                    disallow = !(!touchUp && getChildAt(0).getTop() >= 0 && getFirstVisiblePosition() == 0);
                } else {
                    disallow = false;
                }
