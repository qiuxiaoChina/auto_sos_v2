...
public abstract class PullToRefreshAdapterViewBase<T extends AbsListView>
    ...
    private boolean isFirstItemVisible() {
        ...
        if (adapter == null) {
            ...
            if (mRefreshableView.getChildCount() > 0) {
                if (mRefreshableView.getFirstVisiblePosition() <= 1) {
                    final View firstVisibleChild = mRefreshableView
                            .getChildAt(0);
        ...
        /*
         * if (null == adapter || adapter.isEmpty()) { if (DEBUG) {
		 * Log.d(LOG_TAG, "isFirstItemVisible. Empty View."); } return false;
		 * 
		 * } else {
		 */
         ...
         */
		/*
		 * if (mRefreshableView.getFirstVisiblePosition() <= 1) { final View
		 * firstVisibleChild = mRefreshableView.getChildAt(0); if
		 * (firstVisibleChild != null) { return firstVisibleChild.getTop() >=
		 * mRefreshableView .getTop(); } } }
		 */
