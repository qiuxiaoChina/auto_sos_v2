...
import com.etsy.android.grid.StaggeredGridView.OnScrollDeltaChangedListener;
...
public abstract class ExtendableListView extends AbsListView {
    ...
    protected OnScrollDeltaChangedListener changedListener;
            ...
            if (mItemCount == 0) {
                ...
                throw new IllegalStateException(
                        "The content of the adapter has changed but "
                                ...
                                + "from the UI thread. [in ExtendableListView("
                                + getId() + ", " + getClass()
    ...
    private boolean moveTheChildren(int deltaY, int incrementalDeltaY) {
        ...
        if (changedListener != null) {
            changedListener.onScrollDelta(incrementalDeltaY);
        }
        if (incrementalDeltaY < 0) {
        ...
        if (lastPosition == mItemCount - 1 && childCount > 0) {
            ...
            if (bottomOffset > 0
                ...
                if (mFirstPosition > 0) {
                    ...
                    fillUp(previousPosition,
                            getNextChildUpsBottom(previousPosition));
            ...
            if (topOffset > 0) {
                if (lastPosition < mItemCount - 1 || lastBottom > end) {
                    ...
                    if (lastPosition < mItemCount - 1) {
                        ...
                        fillDown(nextPosition,
                                getNextChildDownsTop(nextPosition));
                        // Close up the remaining gap
                        adjustViewsUpOrDown();
                    }
    ...
    class PositionScroller extends AbsPositionScroller implements Runnable {
        ...
        private void scrollToVisible(int targetPos, int boundPos, int duration) {
            ...
            final int childCount = getChildCount();
            final int lastPos = firstPos + childCount - 1;
            final int paddedTop = mItemMargin;
            final int paddedBottom = getHeight() - mItemMargin;
            ...
            switch (mMode) {
                case MOVE_DOWN_POS: {
                    ...
                    final int extraScroll = lastPos < mItemCount - 1 ?
                            Math.max(mItemMargin, mExtraScroll) : mItemMargin;
                ...
                case MOVE_DOWN_BOUND: {
                    ...
                    final int extraScroll = Math.max(mItemMargin, mExtraScroll);
                ...
                case MOVE_UP_POS: {
                    ...
                    if(mFlingRunnable!=null&&mFlingRunnable.mScroller!=null){
                        isScrolling=!mFlingRunnable.mScroller.isFinished();
                    }
                    if (firstPos == mLastSeenPos&&isScrolling) {
                        // No new views, let things keep going.
                            postOnAnimation(this);
                            return;
                    }
                    ...
                    final int extraScroll = firstPos > 0 ?
                            Math.max(mExtraScroll, mItemMargin) : mItemMargin;

                    smoothScrollBy(firstViewTop*2 - extraScroll, mScrollDuration, true);

                    mLastSeenPos = firstPos;

                    if (firstPos > mTargetPos||(firstPos ==mTargetPos&&firstViewTop<0)) {
                        postOnAnimation(this);
                    }
                    break;
                }
    ...
    private AbsPositionScroller mPositionScroller;
