...
public class StaggeredGridView extends ExtendableListView {
     ...
     * {@link GridItemRecord} with some info about that position so we can
     * maintain it's position - and reorg on orientation change.
     */
    ...
    /**
     * Tells us the distance we've offset from the top. Can be slightly off on
    ...
    public StaggeredGridView(final Context context, final AttributeSet attrs,
        ...
        if (attrs != null) {
            ...
            TypedArray typedArray = context.obtainStyledAttributes(attrs,
                    R.styleable.StaggeredGridView, defStyle, 0);
    ...
    // Grid padding is applied to the list item rows but not the header and
    // footer
    ...
    protected void onMeasure(final int widthMeasureSpec,
        ...
        if (mColumnCount <= 0) {
            ...
            mColumnCount = isLandscape ? mColumnCountLandscape
    ...
    protected void onMeasureChild(final View child,
        ...
        if (viewType == ITEM_VIEW_TYPE_HEADER_OR_FOOTER
            ...
            if (DBG)
                Log.d(TAG, "onMeasureChild BEFORE position:" + position + " h:"
                        + getMeasuredHeight());
            // measure it to the width of our column.
            int childWidthSpec = MeasureSpec.makeMeasureSpec(mColumnWidth,
                    MeasureSpec.EXACTLY);
            int childHeightSpec;
        ...
        if (DBG)
            Log.d(TAG, "onMeasureChild AFTER position:" + position + " h:"
                    + childHeight);
    }
    ...
    protected void onLayoutChild(final View child, final int position,
        ...
        if (isHeaderOrFooter(position)) {
            layoutGridHeaderFooter(child, position, flowDown, childrenLeft,
    ...
    // OR
    // Views will be offset if they are active but offscreen so that we can
    // recycle!
    // Both onLayoutChild() and onOffsetChild are called after we measure our
    // view
    // see ExtensibleListView.setupChild();

    private void layoutGridHeaderFooter(final View child, final int position,
                                        final boolean flowDown, final int childrenLeft, final int childTop,
                                        final int childRight, final int childBottom) {
        ...
        super.onLayoutChild(child, position, flowDown, childrenLeft,
                gridChildTop, childRight, gridChildBottom);
    }

    private void layoutGridChild(final View child, final int position,
                                 final boolean flowDown, final int childrenLeft, final int childRight) {
        ...
        if (flowDown) {
            ...
            // last items bottom
            gridChildBottom = gridChildTop
                    + (getChildHeight(child) + verticalMargins);
        } else {
            gridChildBottom = mColumnTops[column]; // the bottom of the next
            // column up is our top
            gridChildTop = gridChildBottom
                    - (getChildHeight(child) + verticalMargins);
        }

        if (DBG)
            Log.d(TAG, "onLayoutChild position:" + position + " column:"
                    + column + " gridChildTop:" + gridChildTop
                    + " gridChildBottom:" + gridChildBottom);

        // we also know the column of this view so let's stash it in the
        // view's layout params
        GridLayoutParams layoutParams = (GridLayoutParams) child
                .getLayoutParams();
    ...
    protected void onOffsetChild(final View child, final int position,
        ...
        if (isHeaderOrFooter(position)) {
            ...
            offsetGridHeaderFooter(child, position, flowDown, childrenLeft,
                    childTop);
        } else {
    ...
    private void offsetGridHeaderFooter(final View child, final int position,
                                        final boolean flowDown, final int childrenLeft, final int childTop) {
        // offset the top and bottom of all our columns
        // if it's the footer we want it below the lowest child bottom
        int gridChildTop;
        ...
        super.onOffsetChild(child, position, flowDown, childrenLeft,
                gridChildTop);
    }

    private void offsetGridChild(final View child, final int position,
                                 final boolean flowDown, final int childrenLeft, final int childTop) {
        // stash the bottom and the top if it's higher positioned
        int column = getPositionColumn(position);
        ...
        if (DBG)
            Log.d(TAG, "onOffsetChild position:" + position + " column:"
                    + column + " childTop:" + childTop + " gridChildTop:"
    ...
    protected LayoutParams generateChildLayoutParams(final View child) {
        ...
        if (layoutParams == null) {
            layoutParams = new GridLayoutParams(mColumnWidth,
    ...
    /**
     * Get the top for the next child down in our view (maybe a column across)
     * so we can fill down.
     */
    @Override
    ...
    /**
     * Get the bottom for the next child up in our view (maybe a column across)
     * so we can fill up.
     */
    @Override
    ...
    protected void offsetChildrenTopAndBottom(final int offset, final int column) {
        if (DBG)
            Log.d(TAG, "offsetChildrenTopAndBottom: " + offset + " column:"
                    + column);
        final int count = getChildCount();
        for (int i = 0; i < count; i++) {
            final View v = getChildAt(i);
        ...
        if (childCount > 0) {
            for (int i = 0; i < childCount; i++) {
                ...
                if (child != null && child.getLayoutParams() != null
                    ...
                    GridLayoutParams lp = (GridLayoutParams) child
                            .getLayoutParams();
    ...
    protected void onChildrenDetached(final int start, final int count) {
        ...
        for (int i = 0; i < getChildCount(); i++) {
            ...
            if (child != null) {
                final LayoutParams childParams = (LayoutParams) child
                        .getLayoutParams();
                if (childParams.viewType != ITEM_VIEW_TYPE_HEADER_OR_FOOTER
                    ...
                    if (childTop < mColumnTops[column]) {
                        mColumnTops[column] = childTop
                                - getChildTopMargin(position);
                    }
                    final int childBottom = child.getBottom();
                    if (childBottom > mColumnBottoms[column]) {
                        mColumnBottoms[column] = childBottom
                                + getChildBottomMargin();
                    }
                } else {
    ...
    protected void onSizeChanged(int w, int h) {
        ...
        int newColumnCount = isLandscape ? mColumnCountLandscape
                : mColumnCountPortrait;
    ...
    private int calculateColumnWidth(final int gridWidth) {
        ...
        return (gridWidth - listPadding - mItemMargin * (mColumnCount + 1))
                / mColumnCount;
    }

    private int calculateColumnLeft(final int colIndex) {
        return getRowPaddingLeft() + mItemMargin
                + ((mItemMargin + mColumnWidth) * colIndex);
    }

    /**
     * Our mColumnTops and mColumnBottoms need to be re-built up to the
     * mSyncPosition - the following layout request will then layout the that
     * position and then fillUp and fillDown appropriately.
     */
    private void onColumnSync() {
        // re-calc tops for new column count!
        int syncPosition = Math.min(mSyncPosition, getCount() - 1);

        SparseArray<Double> positionHeightRatios = new SparseArray<Double>(
                syncPosition);
        ...
        for (int pos = 0; pos < syncPosition; pos++) {
            ...
            if (isHeaderOrFooter(pos)) {
                ...
                bottom = top + height + getChildTopMargin(pos)
                        + getChildBottomMargin();
            ...
            if (DBG)
                Log.d(TAG, "onColumnSync position:" + pos + " top:" + top
    ...
    private void setPositionHeightRatio(final int position, final int height) {
        ...
        if (DBG)
            Log.d(TAG, "position:" + position + " width:" + mColumnWidth
                    + " height:" + height + " heightRatio:" + rec.heightRatio);
    }
    ...
    public Parcelable onSaveInstanceState() {
        ...
        GridListSavedState ss = new GridListSavedState(
                listState.getSuperState());
    ...
    public void setChangedListener(OnScrollDeltaChangedListener changedListener) {
        this.changedListener = changedListener;
    }

    public interface OnScrollDeltaChangedListener {
        void onScrollDelta(int delte);
    }

    /**
     * Our grid item state record with {@link Parcelable} implementation so we
     * can persist them across the SGV lifecycle.
     */
    static class GridItemRecord implements Parcelable {
        ...
        public String toString() {
            return "GridItemRecord.ListSavedState{"
                    ...
                    + " column:" + column + " heightRatio:" + heightRatio
                    + " isHeaderFooter:" + isHeaderFooter + "}";
        }
    }

    /**
     * Extended LayoutParams to column position and anything else we may been
     * for the grid
     */
    public static class GridLayoutParams extends LayoutParams {
        ...
        /**
         * Here we're making sure that all grid view items are width
         * MATCH_PARENT and height WRAP_CONTENT. That's what this grid is
         * designed for
         */
    ...
    public static class GridListSavedState extends ListSavedState {
        ...
        public GridListSavedState(Parcel in) {
            ...
            positionData = in.readSparseArray(GridItemRecord.class
                    .getClassLoader());
        }
